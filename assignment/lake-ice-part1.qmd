...
ggplot(sunapee, aes(x = year, y = ice_off_doy)) +
  ...
  labs(
    title = "Lake Sunapee Ice-Off Day of Year (1869–Present)",
    x = "Year",
...
The slope estimate for year is -0.106, meaning each year the ice off day is happening earlier.
The multiple R2 is 0.206, meaning roughly 20.6% of the variation in ice off dates is explained by the year.
...
Based on the R2, year and ice_off_day are moderately related, but not a strong relationship.
...
ggplot(sunapee, aes(x = residual)) +
  ...
  labs(
    title = "Histogram of Residuals for Lake Sunapee Ice-Off Regression",
    x = "Residual (Observed - Predicted DOY)",
    y = "Frequency"
  ) +
  theme_minimal()
```
...
sunapee <- sunapee %>%
  ...
  add_residuals(fit, var = "residual")
```
...
The p-value is 2.5e-09, meaning that the negative trend is statistically significant.
...
Slope -> -0.106
Years <- change in days/slope = 7/-0.106 = 66 years

Yes, because a shift of 1 week over multiple decades shows a large climate change. Even though the slope is small, it will accumulate over time.
...
Pretty comfortable using this linear model. Why, because the trend is statistically significant since the slope is negative and the p-value is very small. Residuals are cenntered around zero and normally distributed. Slope is small but accumulates. Moderate R2, showing there's a year to year variability.
...
ggplot(sunapee, aes(x = year, y = ice_off_doy)) +
  ...
  labs(title = "Lake Sunapee Ice Day of Year vs Year",
       x = "Year",
       y = "Ice Day of Year") +
  theme_minimal()
```
...
```{r}
# 1. Predict ice-off day for 2040
pred_2040 <- predict(fit, newdata = data.frame(year = 2040))
pred_2040

# 2. Compute 90% empirical confidence interval from residuals
resid_lower <- quantile(sunapee$residual, 0.05)  # negative value
resid_upper <- quantile(sunapee$residual, 0.95)  # positive value

# 3. Calculate lower and upper bounds for 2040 prediction
ci_lower <- pred_2040 + resid_lower
ci_upper <- pred_2040 + resid_upper
ci_lower
ci_upper

# 4. Width of the confidence interval (in days)
ci_width <- ci_upper - ci_lower
ci_width
```

Predicted ice-off day for 2040: 102.7 (day of year)

90% confidence interval lower bound: 83.2

90% confidence interval upper bound: 116.8

Width of confidence interval: 33.6 days
...
coef_early <- coef(fit_early)
coef_late  <- coef(fit_late)

# Plot points and regression lines
ggplot(sunapee, aes(x = year, y = ice_off_doy)) +
  geom_point(color = "blue") +
  geom_abline(intercept = coef_early[1], slope = coef_early[2], color = "red", size = 1) +
  geom_abline(intercept = coef_late[1], slope = coef_late[2], color = "green", size = 1) +
  labs(title = "Lake Sunapee Ice Day of Year: Early vs Late Periods",
       x = "Year",
       y = "Ice Day of Year") +
  theme_minimal() +
  annotate("text", x = 1950, y = max(sunapee$ice_doy), label = "Before 1970", color = "red") +
  annotate("text", x = 1990, y = max(sunapee$ice_doy), label = "1970 and After", color = "green")
```

The slope of the early model shows a much flatter line, compared to the model after 1970. We can deduce that due to human impact and climate change we see in more recent years, that the ice off day is occuring earlier and earlier each year.
...
https://chatgpt.com/share/699d236c-e67c-800c-aaab-b4ef1e3af5d6
